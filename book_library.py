...
def AllMembersDisplay():
    all_member =Members.query.all() #get data from database
    res=[] #create empty array
    for member in all_member: # loop on array
        res.append({"name":member.member_name,"age":member.member_age,"city":member.member_city,"id":member.member_id}) # add item item from the table to array
    return res # return JSON to clien
...
def add_member(): 
    if request.method== 'POST':
        request_data = request.json
        member_name = request_data['MemName']
        member_age = request_data['MemAge']
        member_city = request_data['MemCity']
